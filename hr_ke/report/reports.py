...
class PayslipReports(models.Model):
    ...
    def GetPayslipExcel(self):
        for rec in self:
            ...
            rec.env['hr.ke'].style_range(ws, 'A8:F13', alignment=al1)
            rec.env['hr.ke'].style_range(ws, 'A8:A13')
            rec.env['hr.ke'].style_range(ws, 'C8:C13')
            rec.env['hr.ke'].style_range(ws, 'E8:E13')
...
class PayrollReports(models.Model):
    ...
    def GetP10(self):
        for rec in self:
            filename_employee = 'KRA_P10-' + re.sub(
                '[^A-Za-z0-9]+', '', rec.name) + '_' + fields.Datetime.context_timestamp(
                self, fields.Datetime.now()).strftime('%Y_%m_%d-%H%M%S') + '.csv'
            filename_disabled = 'Disabled_KRA_P10-' + re.sub(
                '[^A-Za-z0-9]+', '', rec.name) + '_' + fields.Datetime.context_timestamp(
                self, fields.Datetime.now()).strftime('%Y_%m_%d-%H%M%S') + '.csv'
